...
def scale(data):
  ...
  """
  print("\n\n\nNow moving on to scale your data. Scaling your data is important to handle skewness in your numerical columns. Scanning for skewed columns...")
  try: 
    ...
    for col in numerical_cols:
      ...
      if abs(skew) > skewness_threshold:
        ...
        print("\n\n\n\nHere's a brief explanation of the available correction methods:")
          ...
          while True:  # Loop until a valid choice is made
            ...
            if fix_method in ["log", "sqrt"]:
              ...
              if fix_method == "log":
                ...
                print(f"\nApplied log transformation to column '{col}'.")
              else:
                data[col] = np.sqrt(data[col])
                print(f"\nApplied square root transformation to column '{col}'.")
      ...
      if not skewed_cols:
        print(f"\n  - No significant skewness detected in the '{col}' column.\n")

    # User prompt for scaling/normalization (if applicable)
    if len(numerical_cols) > 0 and skewed_cols:
      print("\n\n\n\nHere's a brief explanation of the available scaling/normalization methods:")
        ...
        while True:  # Loop until a valid choice is made
          ...
          if method in ["standard", "minmax"]:
            ...
            if missing_values_found:  
              print("\nWarning: Missing values detected in numerical columns. Consider imputation before scaling.")
              print(f"Missing value counts: {data[numerical_cols].isnull().sum()}")
            else:
            ...
            print(f"\nApplied {method} scaling to numerical features.")
            break  # Exit the loop if a valid choice is made


          elif method == "skip":
            print("\nSkipping scaling/normalization.")
            break
          else:
...
def create_interaction_features(data, categorical_cols=None):
  ...
  """
  print("\n\n\nNow we will move on to create interaction features from categorical columns.\n")
  if categorical_cols is None:
    categorical_cols = [col for col in data.columns if data[col].dtype == 'category']

  if not categorical_cols:
    print("\n  - No categorical columns found in the data. Skipping interaction feature creation.")
    return data

  # Display recommendations before prompting user
  print("\n\n\n\nRecommendations for Interaction Features:")
  ...
  if action != "y":
    print("\nSkipping interaction feature creation.")
    return data
  ...
  while True:
    ...
    if choice in ["all", "specific"]:
      ...
      print("\nInvalid choice. Please choose 'all' or 'specific'.")
  ...
  if choice == "all":
    ...
    print("\nCreated all possible pairwise interaction features.")
    ...
    while True:
      ...
      if col_name == "done":
        if not selected_cols:
          print("\nNo columns selected. Skipping interaction feature creation.")
        else:
          for col1 in selected_cols:
            for col2 in selected_cols:
              if col1 != col2:
                data[f"{col1}_x_{col2}"] = data[col1].astype(str) + "_" + data[col2].astype(str)
          print(f"\nCreated interaction features for selected columns: {', '.join(selected_cols)}")
        break
      elif col_name in categorical_cols:
        selected_cols.append(col_name)
        print(f"Column '{col_name}' added for interaction features.")
      else:
        print(f"\nInvalid column name: '{col_name}'. Please choose from categorical columns.")
...
def create_feature_bins(data, continuous_cols=None, n_bins=5):
  ...
  """
  print("\n\n\nWe will now move onto creating feature bins (intervals) for continuous features.\n")
  if continuous_cols is None:
    continuous_cols = [col for col in data.columns if data[col].dtype in ['float64', 'int64']]

  if not continuous_cols:
    print("\  - nNo continuous features found in the data. Skipping binning.")
    return data
  ...
  if choice == "all":
    ...
    while True:
      ...
      if col_name == "done":
        if not selected_cols:
          print("\nNo columns selected. Skipping binning.")
        else:
        ...
        print(f"\nInvalid column name: '{col_name}'. Please choose from continuous features.")
...
def create_custom_features(data):
  ...
  while True:
    ...
    if choice in ["expression", "interaction", "none"]:
      ...
      print("\nInvalid choice. Please choose 'expression', 'interaction', or 'none'.")
  ...
  if choice == "expression":
    ...
    while True:
      ...
      try:
        ...
        print("\nInvalid expression. Please use existing feature names and basic mathematical operators (+, -, *, /).")
        continue
      ...
      try:
        ...
        print(f"\nCreated new feature: '{new_feature_name}'")
        break  # Exit the loop if expression is valid
      except (NameError, SyntaxError):
        print("\nError evaluating expression. Please check for typos or invalid syntax.")
    ...
    print("\nSkipping custom feature creation.")
...
def create_one_hot_encoding(data, categorical_cols=None):
  ...
  """
  print("\n\n\nNow we can move onto creating one-hot encoded features from your data's categorical columns.")
  if categorical_cols is None:
  ...
  print("\nGeneral summary of one-hot encoding technique:")
  ...
  if choice == "all":
    ...
    print("\nCreated one-hot encoded features for all categorical columns.")
    ...
    while True:
      ...
      if col_name == "done":
        if not selected_cols:
          print("\nNo columns selected. Skipping one-hot encoding.")
        else:
          data = pd.get_dummies(data, columns=selected_cols, drop_first=True)
          print(f"\nCreated one-hot encoded features for selected columns.")
        break
      elif col_name in categorical_cols:
        selected_cols.append(col_name)
        print(f"Feature '{col_name}' added for one-hot encoding.")
      else:
        print(f"\nInvalid column name: '{col_name}'. Please choose from categorical features.")
...
def create_label_encoding(data, categorical_cols=None):
  ...
  print("\n\n\nNow we will move on to creating label encoded features from you data's categorical columns.\n")
  if categorical_cols is None:
    categorical_cols = [col for col in data.columns if data[col].dtype == "object"]
  ...
  print("\nLabel encoding is a simpler way to handle categorical features. It assigns a unique number to each different category.")
  ...
  if choice == "all":
    ...
    print("\nCreated label encoded features for all categorical columns.")
    ...
    while True:
      ...
      if col_name == "done":
        if not selected_cols:
          print("\nNo columns selected. Skipping label encoding.")
        else:
          for col in selected_cols:
            le = sklearn.preprocessing.LabelEncoder()
            data[col] = le.fit_transform(data[col])
          print(f"\nCreated label encoded features for selected columns.")
        break
      elif col_name in categorical_cols:
        selected_cols.append(col_name)
...
def handle_class_imbalance(data, target_column):
  ...
  print("\n\n\nNow we will move on to handle class imbalances in your dataset.")
  ...
  print("\nYour target variable's class distribution:")
  ...
  if not imbalanced:
    print("\nYour class distribution is balanced. Skipping imbalance handling.")
    return data



  # Get user choice for handling imbalance
  print("\nYour target variable exhibits a class imbalance. Here are some techniques to handle the imbalance:")
  ...
  while True:
    ...
    if choice in ["undersample", "oversample", "none"]:
      ...
      print("\nInvalid choice. Please choose 'undersample', 'oversample', or 'none'.")
  ...
  if choice in ["undersample", "oversample"]:
    ...
    while True:
      ...
      if sampling_ratio > 0 or sampling_ratio <= 1:
        ...
        print("\nInvalid sampling ratio. Please enter a value between 0 and 1.")
    ...
    if choice == "undersample":
      ...
      print(f"\nUndersampled majority class to {int(sampling_ratio * majority_count)} samples.")
    else:
      sm = SMOTE(sampling_strategy={target_column: "auto"})
      data = sm.fit_resample(data, data[target_column])
      print(f"\nOversampled minority class to match the majority class size.")
...
def feature_selection(data, target_column):
  ...
  print("\n\n\nNow we will move onto feature selection.")
  ...
  print("\nThe feature selection method we us is a 'Select K Best' filter method, which means that we will use your chosen number of")
  print("strongest predictive features in your dataset (based on statistical tests for individual features).")

  while True:
    choice = input("Do you want to proceed with this method to select features to model? If 'n', we will use all features to model. (y/n)").lower()
    if choice in ["y", "n"]:
      break
    else:
  ...
  print("\nWe will use the 'Select K best' filter method. This means we will use your chosen number of the most predictive features in your dataset.")
  ...
  if choice == "n":
    print("\nOriginal features will be used for model training.")
    return data
